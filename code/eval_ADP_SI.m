function [Y] = eval_ADP_SI(params, iemg_prof, cycles, cut_off, ADP_set, dpidt_set, dHdt_set, dMgADPdt_set, dPCrdt_set)
...
for i=1:m
    ...
    if i<=cut_off 
        ...
        init(13) = ADP_set*ADP_p(cut_off);
        [T, Y] = ode15s(@Model_XB_human_QC_metdyn_set_SI,tspan,init,options,TmpC,SL_set,params,iemg,dSL_set,Pcr,H,dpidt_set,dHdt_set,dMgADPdt_set,dPCrdt_set);
    end
    init(10)=Y(n,10);%N
    init(12)=Y(n,12);%P
    init(14)=Y(n,14);%Pcr
    ...
    ATP_p(i)=Y(n,16);
    for j=1:n
        [~, sim_Ftotal_cycles(j,i),~,~,~,~,~,~,~,~,~,~,~,~,~] = Model_XB_human_QC_SI(T(j),Y(j,:),TmpC,SL_set,params,iemg,dSL_set,Pcr,H);
    end
