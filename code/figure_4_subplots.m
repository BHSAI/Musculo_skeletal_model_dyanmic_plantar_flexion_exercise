...
%rmsd_check = zeros(length(y_labels),1);
for i=1:q
    figure(i);clf;
    ...
    if exp_flag(i)>0
        ...
        writematrix(table_temp,fullfile(pwd,'Figure_4_subplots',filename2{i}));
    end
    if exp_flag(i)>0
    ...
    exportgraphics(figure(i),fullfile('figure_4_subplots',filename{i}),'BackgroundColor','w','Resolution',300,'ContentType','vector');   
end
...
writetable(t_rmsd,fullfile(pwd,'figure_4_subplots','RMSD_fig1.xlsx'),'WriteRowNames',true);
writetable(frac_table,fullfile(pwd,'figure_4_subplots','frac_fig1.xlsx'),'WriteRowNames',true);
%plot power data
figure(q+1);clf;
...
p = get(gca,'Position');
set(gca,'Unit','Inches','Position',[p(1) p(2) 1.75 1.25]);
exportgraphics(figure(q+1),fullfile(pwd,'figure_4_subplots','Force_mod.pdf'),'BackgroundColor','w','Resolution',300,'ContentType','vector');